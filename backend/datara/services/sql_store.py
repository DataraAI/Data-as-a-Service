...
class SQLStore:
    ...
    def initialize(self) -> None:
        with self.connection() as conn:
            conn.executescript(
                ...
                CREATE TABLE IF NOT EXISTS occlusion_jobs (
                    id TEXT PRIMARY KEY,
                    ...
                    output_route_path TEXT,
                    output_viewer_path TEXT,
                    error_message TEXT,
                    worker_pid INTEGER,
                    created_at TEXT NOT NULL,
                    updated_at TEXT NOT NULL,
                    started_at TEXT,
                    finished_at TEXT,
                    FOREIGN KEY(submitted_by_user_id) REFERENCES users(entra_object_id),
                    FOREIGN KEY(dataset_id) REFERENCES datasets(id)
                );

                CREATE INDEX IF NOT EXISTS idx_occlusion_jobs_status_created
                    ON occlusion_jobs(status, created_at);
                CREATE INDEX IF NOT EXISTS idx_occlusion_jobs_submitter_created
                    ON occlusion_jobs(submitted_by_user_id, created_at);
                """
            )
    ...
    def create_occlusion_job(
        ...
        *,
        ...
        dataset_id: str | None,
        request_json: str,
        output_route_path: str | None = None,
        ...
        with self.connection() as conn:
            conn.execute(
                ...
                VALUES (?, ?, ?, ?, ?, 'queued', 'queued', 0, 0, ?, ?, NULL, NULL, ?, ?, NULL, NULL)
                """,
                (
                    job_id,
                    submitted_by_user_id,
        ...
        return self.get_occlusion_job(job_id)

    def get_occlusion_job(self, job_id: str) -> dict[str, Any] | None:
        return self._fetchone("SELECT * FROM occlusion_jobs WHERE id = ?", (job_id,))

    def update_occlusion_job(self, job_id: str, **fields: Any) -> dict[str, Any] | None:
        if not fields:
            return self.get_occlusion_job(job_id)
        ...
        with self.connection() as conn:
            conn.execute(
                f"UPDATE occlusion_jobs SET {assignments} WHERE id = ?",
                tuple(values),
            )
        return self.get_occlusion_job(job_id)

    def claim_next_occlusion_job(self, *, worker_pid: int) -> dict[str, Any] | None:
        with self.connection() as conn:
            conn.execute("BEGIN IMMEDIATE")
            running = conn.execute(
                "SELECT id FROM occlusion_jobs WHERE status = 'running' ORDER BY created_at ASC LIMIT 1"
            ).fetchone()
            if running is not None:
                return None

            row = conn.execute(
                "SELECT * FROM occlusion_jobs WHERE status = 'queued' ORDER BY created_at ASC LIMIT 1"
            ).fetchone()
            if row is None:
                return None
            ...
            claimed = conn.execute(
                "SELECT * FROM occlusion_jobs WHERE id = ?",
                (row["id"],),
            ).fetchone()
            return row_to_dict(claimed)

    def complete_occlusion_job(
        self,
        job_id: str,
        *,
        output_route_path: str,
        output_viewer_path: str,
    ) -> dict[str, Any] | None:
        now = utc_now()
        with self.connection() as conn:
            conn.execute(
                ...
                WHERE id = ?
                """,
                (output_route_path, output_viewer_path, now, now, job_id),
            )
        return self.get_occlusion_job(job_id)

    def fail_occlusion_job(self, job_id: str, error_message: str) -> dict[str, Any] | None:
        now = utc_now()
        with self.connection() as conn:
            conn.execute(
                """
    ...
    def fail_running_occlusion_jobs(self, error_message: str) -> int:
        now = utc_now()
        with self.connection() as conn:
            cursor = conn.execute(
                """
