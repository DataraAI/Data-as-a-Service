...
import {
    VideoToolsPanel,
    type VideoFolderAsset,
} from "../components/VideoToolsPanel";
...
function cloneEntryWithTitle(
    entry: ResolvedCatalogCardEntry,
    title: string,
): ResolvedCatalogCardEntry {
    return {
        ...entry,
        card: {
            ...entry.card,
...
function getSuggestionScore(fullPath: string, query: string): number | null {
    ...
    segments.forEach((segment, index) => {
        ...
        if (queryTerms.some((term) => segment.startsWith(term))) {
            bestStartsWithDistance = Math.min(
                bestStartsWithDistance,
                distanceFromEnd,
            );
        }
        if (queryTerms.some((term) => segment.includes(term))) {
            bestIncludesDistance = Math.min(
                bestIncludesDistance,
                distanceFromEnd,
            );
        }
    });

    if (bestStartsWithDistance !== Number.POSITIVE_INFINITY)
        return 10 + bestStartsWithDistance;
    if (bestIncludesDistance !== Number.POSITIVE_INFINITY)
        return 20 + bestIncludesDistance;

    return 50 + segments.length;
}

function getBestMatchingSegmentIndex(
    fullPath: string,
    query: string,
): number | null {
    const normalizedQuery = normalizePathSearchValue(query);
    ...
    segments.forEach((segment, index) => {
        ...
        if (normalizedQuery && normalizedSegment === normalizedQuery) {
            ...
            normalizedSegment.startsWith(normalizedQuery)
        ) {
            rank = 100 + distanceFromEnd;
        } else if (
            normalizedQuery.length >= 2 &&
            normalizedSegment.includes(normalizedQuery)
        ) {
...
function getCategoryByRouteKey(value?: string | null) {
    ...
    return (
        CATEGORIES.find((category) => {
            if (normalizeCategoryValue(category.routeKey) === normalizedValue)
                return true;
            if (normalizeCategoryValue(category.publicSlug) === normalizedValue)
                return true;
            return category.aliases.some(
                (alias) => normalizeCategoryValue(alias) === normalizedValue,
            );
        }) ?? null
    );
}
...
function pathBelongsToCategory(fullPath: string, routeKey: CategoryKey) {
    ...
    return (
        ...
        normalizeCategoryValue(categorySegment) ===
            normalizeCategoryValue(routeKey)
    );
}
...
function getPreviewDatasetRootFromVideoBlobPath(blobPath?: string | null) {
    const normalized = String(blobPath ?? "")
        .trim()
        .replace(/\\/g, "/");
    if (!normalized) return null;
    const match = normalized.match(
        /^(.*)\/preview\/hover\.(mp4|webm|mov|m4v)$/i,
    );
    return match?.[1] ?? null;
}
...
function buildCategoryLandingPath(category: CategoryConfig, basePath: string) {
    const segment = basePath.startsWith("/robodatahub")
        ? category.publicSlug
        : category.routeKey;
...
function getCategoryBadge(card: CatalogCard) {
    const isEgo = card.tags.some((tag) =>
        tag.toLowerCase().includes("ego-centric"),
    );
    return {
        label: isEgo ? "EGO" : "EXO",
        className: isEgo
...
function matchesLivePreviewHint(
    card: CatalogCard,
    ...
    return card.livePathHints.some((hint) =>
        normalizedPath.includes(normalizePathSearchValue(hint)),
    );
}
...
function buildFallbackLiveCard(item: CategoryDatasetPreview): CatalogCard {
    const label =
        item.title?.trim() ||
        item.full_path.split("/").filter(Boolean).pop() ||
        item.full_path;
...
function buildLocalPreviewAsset(
    blobPath: string,
    label: string,
    index: number,
): CategoryPreviewAsset {
...
function buildLocalPlaceholderPreview(
    ...
    return {
        ...
        main_image: buildLocalPreviewAsset(
            usableImages[0],
            `${snapshot.dataset} cover`,
            0,
        ),
        thumbnails: usableImages
            ...
            .map((blobPath, index) =>
                buildLocalPreviewAsset(
                    blobPath,
                    `${snapshot.dataset} preview ${index + 1}`,
                    index + 1,
                ),
            ),
        preview_video: null,
...
function resolveCatalogSectionsForCategory(
        ...
        section.cards.forEach((card) => {
            const placeholderItem =
                localCategoryPreviewPlaceholders.find((item) => {
                    const normalizedPath = normalizePathSearchValue(
                        item.full_path,
                    );
                    return (
                        !usedPlaceholderPaths.has(normalizedPath) &&
                        matchesLivePreviewHint(card, item)
                    );
                }) ?? null;

            if (placeholderItem) {
                usedPlaceholderPaths.add(
                    normalizePathSearchValue(placeholderItem.full_path),
                );
            }
            ...
            if (matchedItem) {
                usedLivePaths.add(
                    normalizePathSearchValue(matchedItem.full_path),
                );
            }

            sectionEntries.push({
                card,
                liveItem: matchedItem,
                placeholderItem,
            });
        });
    ...
    const upsertGroupedExtra = (
        ...
        const baseCard = buildFallbackLiveCard(
            existingEntry?.liveItem ?? existingEntry?.placeholderItem ?? item,
        );

        sectionEntries.set(normalizedPath, {
            card: baseCard,
            liveItem:
                kind === "live" ? item : (existingEntry?.liveItem ?? null),
            placeholderItem:
                kind === "placeholder"
                    ? item
                    : (existingEntry?.placeholderItem ?? null),
        });
    };

    unmatchedPlaceholderItems.forEach((item) => {
        upsertGroupedExtra(
            getLivePreviewSectionId(routeKey, item),
            item,
            "placeholder",
        );
    });
    ...
    return landingContent.sections.map((section) => {
        ...
        const liveBackedCards = resolvedCards.filter(
            (entry) =>
                entry.card.livePathHints?.length ||
                entry.liveItem ||
                entry.placeholderItem,
        );
        const marketingCards = resolvedCards.filter(
            (entry) => !entry.card.livePathHints?.length,
        );
        const extraLiveCards = Array.from(
            groupedExtras.get(section.id)?.values() ?? [],
        ).sort((a, b) =>
            (
                a.liveItem?.title ??
                a.placeholderItem?.title ??
                a.card.title
            ).localeCompare(
        ...
        const uniqueCards = [
            ...
            .map((entry) => {
                ...
                return cloneEntryWithTitle(
                    entry,
                    `${entry.card.title} ${seenCount + 1}`,
                );
            });
...
function getLivePreviewSectionId(
    routeKey: CategoryKey,
    item: CategoryDatasetPreview,
) {
    const normalizedPath = normalizePathSearchValue(item.full_path);
    ...
    switch (routeKey) {
        case "serverrack":
            if (
                normalizedPath.includes("ethernetcable") ||
                normalizedPath.includes("adpluggingcable")
            ) {
                return "cable";
            }
            if (
        ...
        case "warehouse":
            if (
                normalizedPath.includes("loadingpellets") ||
                normalizedPath.includes("steelpallets")
            ) {
                return "material";
            }
            return "pick";
        case "dexterity":
            if (
                normalizedPath.includes("dishwasherunloading") ||
                normalizedPath.includes("peelingpeas")
            ) {
                return "kitchen";
            }
            if (
...
function CuratedCatalogCard({
    ...
    const previewAssets = previewItem
        ? [previewItem.main_image, ...previewItem.thumbnails]
              .filter((asset): asset is CategoryPreviewAsset => Boolean(asset))
              .slice(0, 4)
        : [];
    const fallbackThumbs = [displayImages.main, ...displayImages.thumbs]
        .filter(Boolean)
        .slice(0, 4);
    const mosaicCells = Array.from({ length: 4 }, (_, index) => {
        if (previewAssets.length > 0) {
            return (
                previewAssets[index] ??
                previewAssets[previewAssets.length - 1] ??
                null
            );
        }
        return (
            fallbackThumbs[index] ??
            fallbackThumbs[fallbackThumbs.length - 1] ??
            null
        );
    });
...
function getCatalogAvailabilityClasses(
    availability: CatalogCard["availability"],
) {
...
function RootShowcaseCatalogCard({
    ...
    const previewImageSrc = displayItem?.main_image
        ? resolvePreviewMediaUrl(displayItem.main_image)
        : null;
    const imageSrc = imageFailed
        ? fallbackImageSrc
        : (previewImageSrc ?? fallbackImageSrc);
    const videoSrc = videoFailed
        ? null
        : resolvePreviewMediaUrl(liveItem?.preview_video);
    ...
    return (
        ...
        >
            <div
                ...
                onMouseEnter={
                    videoSrc ? () => setPreviewVideoActive(true) : undefined
                }
                onMouseLeave={
                    videoSrc ? () => setPreviewVideoActive(false) : undefined
                }
            >
                {imageSrc ? (
                    <img
            ...
            <div className="flex flex-1 flex-col p-3">
                ...
                <p className="mt-1 text-[11px] leading-[1.5] text-slate-500">
                    {card.description}
                </p>
                        ...
                        <div className="divide-y divide-border">
                            {suggestions.map((suggestion) => (
                                <button
                                    ...
                                    type="button"
                                    onClick={() =>
                                        onSuggestionClick(suggestion.full_path)
                                    }
...
function getCatalogFilterTargetSectionId(
    ...
    const exactSection = landing.sections.find(
        (section) => section.id === filterId,
    );
    ...
    landing.sections.forEach((section) => {
        ...
        const score = filterTerms.reduce((total, term) => {
            ...
            const isHeadingMatch =
                section.id === term ||
                normalizeCatalogMatchText(section.title).includes(term);
...
export default function DataViewer() {
    ...
    >([]);
    const [loadedCategoryPreviewKey, setLoadedCategoryPreviewKey] =
        useState<CategoryKey | null>(null);
    const [categoryPreviewsLoading, setCategoryPreviewsLoading] =
        useState(false);
    ...
    const [folderDropdownOpen, setFolderDropdownOpen] = useState<string | null>(
        null,
    );
    const [deleteModalFolder, setDeleteModalFolder] =
    ...
    const [visiblePrimitives, setVisiblePrimitives] = useState<Set<string>>(
        new Set(),
    );
    ...
    const pathSegments = useMemo(
        () =>
            location.pathname
                ...
                .filter(
                    (part) =>
                        part && part !== "viewer" && part !== "robodatahub",
                ),
        [location.pathname],
    );
    ...
    const activeLandingContent = useMemo<CategoryLandingContent | null>(
        () =>
            activeCategory
                ? CATEGORY_LANDING_CONTENT[activeCategory.routeKey]
                : null,
        [activeCategory],
    );
        ...
        return CATEGORIES.reduce(
            (acc, category) => {
                acc[category.routeKey] = snapshots
                    ...
                    .map((snapshot) =>
                        buildLocalPlaceholderPreview(snapshot, viewerBasePath),
                    )
                    .filter((item): item is CategoryDatasetPreview =>
                        Boolean(item),
                    );
                return acc;
            },
    ...
    const rootResolvedShowcaseSections = useMemo(() => {
        return ROOT_SHOWCASE_SECTIONS.map((section) => {
            ...
            const categoryPreviewsForRoute =
                rootCategoryPreviews[section.routeKey] ?? [];
            const placeholdersForRoute =
                localCategoryPreviewPlaceholdersByRoute[section.routeKey] ?? [];
            ...
            const flattenedEntries = resolvedSections.flatMap(
                (resolvedSection) => resolvedSection.cards,
            );
            ...
            const cards =
                realBackedEntries.length >= 4
                    ...
                    : [
                          ...realBackedEntries,
                          ...marketingEntries.slice(
                              0,
                              4 - realBackedEntries.length,
                          ),
                      ];
    ...
    const isCategoryLanding =
        Boolean(activeCategory) && pathSegments.length === 1;
    ...
    const handleLandingFilterSelect = (filterId: string) => {
        ...
        const targetSectionId = getCatalogFilterTargetSectionId(
            activeLandingContent,
            filterId,
        );
    ...
    useEffect(() => {
        ...
        async function loadCurrentPath() {
            ...
            try {
                const folderResponse = await axios.get<unknown[]>(
                    "/api/datasets",
                    {
                ...
                const nextFolders = uniqueFolderItems(
                    normalizeFolderResults(folderResponse.data).sort((a, b) =>
                ...
                const imageResponse = await axios.get<ImageItem[]>(
                    `/api/dataset/${encodedPath}`,
                );
                if (cancelled) return;

                const nextImages = Array.isArray(imageResponse.data)
                    ? imageResponse.data
                    : [];
    ...
    useEffect(() => {
        ...
        async function loadCategoryPreviews() {
            ...
            try {
                ...
                if (!cancelled) {
                    console.error(
                        "Failed to load category dataset previews",
                        error,
                    );
                    setCategoryPreviews([]);
                    setLoadedCategoryPreviewKey(null);
                ...
                CATEGORIES.map(async (category) => {
                    try {
                        ...
                        setRootCategoryPreviews((previous) => ({
                            ...previous,
                            [category.routeKey]: Array.isArray(response.data)
                                ? response.data
                                : [],
                        }));
                    } catch (error) {
                        if (!cancelled) {
                            console.error(
                                `Failed to load root dataset previews for ${category.routeKey}`,
                                error,
                            );
                            setRootCategoryPreviews((previous) => ({
                                ...previous,
    ...
    useEffect(() => {
        const shouldLoadPaths =
            isRootLanding || isCategoryLanding || pathSearchTouched;
        ...
        async function loadAllPaths() {
            ...
            try {
                ...
                const nextPaths = uniqueFolderItems(
                    normalizeFolderResults(response.data).sort((a, b) =>
    ...
    useEffect(() => {
        ...
        const match = images.find(
            (image) => image.name === decoded || image.id.endsWith(decoded),
        );
        if (match) {
    ...
    const isMaskPath = useMemo(
        () =>
            pathSegments
                .slice(datasetRootDepth)
                .some((segment) => segment.toLowerCase() === "masks"),
    ...
    const isEgoPath = useMemo(
        () =>
            pathSegments
                .slice(datasetRootDepth)
                .some((segment) => segment.toLowerCase() === "egos"),
        [datasetRootDepth, pathSegments],
    );
    const isOrigPath = useMemo(
        () =>
            pathSegments
                .slice(datasetRootDepth)
                .some((segment) => segment.toLowerCase() === "orig"),
        [datasetRootDepth, pathSegments],
    );
    const showEgocentricGeneration = useMemo(() => {
        ...
        return sourceImages.every((image) => {
            const view = String(image.metadata?.view ?? "")
                .trim()
                .toLowerCase();
            return view === "exo";
        });
    ...
    const allSelectableTags = useMemo(
        () =>
            [
                ...availableTags,
                ...vlmPromptGroups.flatMap((group) => group.tags),
    ...
    const filteredImages = useMemo(() => {
        ...
        return images.filter((image) => {
            ...
            const matchesTags =
                visibleTags.size === 0 ||
                Array.from(visibleTags).every((tag) => imageTags.includes(tag));
    ...
    const videoPanelItems = useMemo<VideoFolderAsset[]>(() => {
        const panelVideos =
            filteredVideos.length > 0 ? filteredVideos : sourceVideos;
    ...
    const pathSuggestions = useMemo(() => {
        ...
        return allFolderPaths
            ...
            .filter(
                (entry): entry is { item: FolderItem; score: number } =>
                    entry.score !== null,
            )
            .sort(
                (a, b) =>
                    a.score - b.score ||
                    a.item.full_path.localeCompare(b.item.full_path),
            )
            .slice(0, 8)
    ...
    function handleCuratedCardOpen(
        pathLabel: string,
    ...
    async function handleDeleteFolder() {
        ...
        try {
            await axios.post("/api/delete_dataset", {
                path: deleteModalFolder.full_path,
            });
            ...
            <div className="font-sans-tech text-sm text-foreground">
                {segments.map((segment, index) => {
                    ...
                    const isMatched = terms.some((term) =>
                        normalizedSegment.includes(term),
                    );
    ...
    function renderFolderGrid(items: FolderItem[], maxWidthClassName: string) {
        return (
            ...
            >
                {items.map((folder) => {
                    const isOwnFolder =
                        folder.full_path.startsWith("my/") ||
                        folder.full_path.split("/").filter(Boolean).length >= 4;
                                    ...
                                    {isOwnFolder && (
                                        <div className="absolute right-0 top-0">
                                            ...
                                            {folderDropdownOpen ===
                                                folder.full_path && (
                                                <div
                                                    className="absolute right-0 top-full z-20 mt-1 w-40 rounded-2xl border border-slate-200 bg-white py-1 shadow-[0_20px_40px_rgba(15,23,42,0.12)]"
                                                    onMouseDown={(event) =>
                                                        event.stopPropagation()
                                                    }
                                                >
                ...
                <div className="grid lg:grid-cols-[220px_minmax(0,1fr)]">
                    <aside className="flex flex-col border-b border-slate-200 bg-slate-50/90 p-5 lg:border-b-0 lg:border-r lg:p-6">
                        <div className="border-b border-slate-200 pb-5">
                            <div className="text-lg font-extrabold tracking-[0.04em] text-primary">
                                DataraAI
                            </div>
                            ...
                            <div className="space-y-1.5">
                                {ROOT_SHOWCASE_SECTIONS.map((section) => {
                                    const category = CATEGORIES.find(
                                        (item) =>
                                            item.routeKey === section.routeKey,
                                    );
                                ...
                                return (
                                    <section key={section.routeKey}>
                                        <div className="mb-4 flex items-center gap-3">
                                            <span
                                                className={`h-3 w-3 shrink-0 rounded-[3px] ${section.dotClassName}`}
                                            />
                                ...
                                <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
                                    <div className="max-w-3xl">
                                        ...
                                        <p className="mt-3 text-sm leading-7 text-slate-600">
                                            Don&apos;t see what you need? Our
                                            team captures any task, environment,
                                            or robot workflow on demand.
                                        </p>
                                    </div>
                                    <div className="flex flex-wrap items-center gap-3">
    ...
    const renderCategoryLanding = (category: CategoryConfig) => {
        ...
        const heroPreviewDatasetRoot = getPreviewDatasetRootFromVideoBlobPath(
            landing.heroVideoBlobPath,
        );
        const heroPlaceholderItem = heroPreviewDatasetRoot
            ? ((
                  localCategoryPreviewPlaceholdersByRoute[category.routeKey] ??
                  []
              ).find(
                  (item) =>
                      normalizePathSearchValue(item.full_path) ===
                      normalizePathSearchValue(heroPreviewDatasetRoot),
              ) ?? null)
            : null;
        const heroPosterUrl =
            (heroPlaceholderItem?.main_image
                ? resolvePreviewMediaUrl(heroPlaceholderItem.main_image)
                : null) ??
                        ...
                        <div className="relative grid gap-10 p-6 md:p-8 lg:grid-cols-2 lg:items-center">
                            ...
                            <div className="relative overflow-hidden rounded-[20px] border border-slate-300 shadow-[0_28px_64px_rgba(15,23,42,0.12)]">
                                ...
                                <div className="absolute bottom-5 left-5 rounded-[12px] border border-slate-200 bg-white/90 px-4 py-3 backdrop-blur-sm">
                                    ...
                                    <div className="mt-1 text-[13px] font-bold text-slate-950">
                                        {landing.heroTitle}
                                    </div>
                                </div>
                    ...
                    <section className="border-b border-slate-200 bg-slate-50/80 px-6 py-5 md:px-8">
                        <div className="grid gap-px overflow-hidden rounded-[16px] border border-slate-300 bg-slate-200 md:grid-cols-5">
                            {landing.stats.map((stat) => (
                                <div
                                    key={`${landing.routeKey}-band-${stat.label}`}
                        ...
                        <aside className="flex flex-col border-b border-slate-200 bg-slate-50/90 p-5 lg:border-b-0 lg:border-r lg:p-6">
                            <CategorySidebarSection
                                ...
                                items={CATEGORIES.map((item) => ({
                                    id: item.routeKey,
                                    label: item.label,
                                    dotClassName: getCategoryAccent(
                                        item.routeKey,
                                    ).dot,
                                }))}
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        {user?.role === "admin" ? (
                                            <button
                                                ...
                                                onClick={() =>
                                                    navigate(
                                                        `${viewerBasePath}/admin/${user.storageSlug}`,
                                                    )
                                                }
                                    ...
                                    <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
                                        <div className="max-w-2xl">
                                            ...
                                            <p className="mt-1 text-sm leading-6 text-slate-500">
                                                Search by dataset, task, or
                                                brand and jump straight into the
                                ...
                                <div className="flex min-h-10 flex-wrap items-center justify-between gap-3 border-b border-slate-200 bg-slate-50/70 px-4 py-2 sm:flex-nowrap sm:py-0">
                                    <div className="flex items-center space-x-4">
                                        <div className="flex items-center rounded-sm border border-slate-200 bg-white px-2 py-1 text-xs shadow-[0_8px_18px_rgba(15,23,42,0.04)]">
                                            <span className="mr-2 font-sans-tech text-slate-500">
                                                Items:
                                            </span>
                                            <span className="font-sans-tech text-slate-900">
                                                {itemCount}
                                            </span>
                                        </div>
