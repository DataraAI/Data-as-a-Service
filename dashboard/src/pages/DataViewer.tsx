...
export default function DataViewer() {
    ...
    const matchingTagSuggestions = useMemo(() => {
        ...
        if (!query) return [];
        return availableTags.filter(tag => !visibleTags.has(tag)).filter(tag => tag.toLowerCase().startsWith(query)).sort((a, b) => a.localeCompare(b));
            ...
            axios.get<ImageItem[]>(`/api/dataset/${currentBackendPath}`)
                .then(res => {
                    ...
                    res.data.forEach((img) => {
                        (img.tags || []).forEach((tag: string) => { if (!tag.includes(': ')) normalTags.add(tag); });
                    ...
                    setVlmPromptGroups(Array.from(promptGroups.entries()).map(([prompt, tags]) => ({ prompt, tags: Array.from(tags).sort((a, b) => a.localeCompare(b)) })).sort((a, b) => a.prompt.localeCompare(b.prompt)));
    ...
    const togglePrimitive = (prim: string) => { const newVisible = new Set(visiblePrimitives); newVisible.has(prim) ? newVisible.delete(prim) : newVisible.add(prim); setVisiblePrimitives(newVisible); };
    const handleFolderClick = (folderName: string) => navigate(location.pathname.endsWith('/') ? `${location.pathname}${folderName}` : `${location.pathname}/${folderName}`);
    ...
    const filteredImages = useMemo(() => {
        ...
        if (frameRange.min !== null || frameRange.max !== null) {
            result = result.filter((img) => {
                ...
                else if (typeof rawFrameId === 'string') { const parsed = Number.parseInt(rawFrameId, 10); if (!Number.isNaN(parsed)) frameId = parsed; }
    ...
    const filteredFolders = useMemo(() => !filterText ? folders : folders.filter((folder) => folder.name.toLowerCase().includes(filterText.toLowerCase())), [folders, filterText]);
        ...
        <div className="flex flex-col h-screen text-foreground bg-background font-sans-tech overflow-hidden relative">
            ...
            <div className="flex flex-col flex-1 overflow-hidden pt-16 relative z-10">
                <div className="h-12 bg-background/80 backdrop-blur-md border-b border-border flex items-center px-4 justify-between z-20 shrink-0">
                    <div className="flex items-center gap-4"><div className="flex items-center gap-2 text-sm"><Link to="/viewer" className="font-sans-tech font-bold text-primary hidden md:block hover:text-primary-glow transition-colors">DATARA EXPLORER</Link><Breadcrumbs /></div></div>
                ...
                {selectedImage && <ImageModal image={selectedImage} onClose={() => setSelectedImage(null)} onNext={() => { const idx = filteredImages.indexOf(selectedImage); if (idx < filteredImages.length - 1) setSelectedImage(filteredImages[idx + 1]); }} onPrev={() => { const idx = filteredImages.indexOf(selectedImage); if (idx > 0) setSelectedImage(filteredImages[idx - 1]); }} />}
                <UploadModal isOpen={isUploadModalOpen} onClose={() => setIsUploadModalOpen(false)} onSuccess={() => { }} />
                {deleteModalFolder && <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm p-4"><div className="bg-card border border-border rounded-lg shadow-xl max-w-md w-full p-6" onClick={(e) => e.stopPropagation()}><h3 className="text-lg font-sans-tech font-bold text-foreground mb-2">Delete path and subdirectories?</h3><p className="text-sm text-muted-foreground font-sans-tech mb-1">You are about to delete <span className="text-foreground font-medium">{deleteModalFolder.full_path}</span> and all of its contents.</p><p className="text-xs text-destructive/90 font-sans-tech mb-6">This action cannot be undone.</p><div className="flex justify-end gap-3"><button type="button" onClick={() => setDeleteModalFolder(null)} className="px-4 py-2 text-sm font-sans-tech font-medium text-muted-foreground hover:text-foreground border border-border rounded-sm transition-colors">Cancel</button><button type="button" onClick={handleDeleteFolder} disabled={deleteInProgress} className="px-4 py-2 text-sm font-sans-tech font-medium text-primary-foreground bg-destructive hover:bg-destructive/90 rounded-sm transition-colors disabled:opacity-50 flex items-center gap-2">{deleteInProgress && <Loader2 className="w-4 h-4 animate-spin" />}Delete</button></div></div></div>}
            </div>
        </div>
    );
}
