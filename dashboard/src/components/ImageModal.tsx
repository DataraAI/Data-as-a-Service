...
function sourceDatasetName(image: any) {
    return (
        image?.dataset?.viewer_path?.split("/").filter(Boolean).pop() ||
        image?.name ||
        "dataset"
    );
}
...
export function ImageModal({
    ...
    const [selectedVlmPreset, setSelectedVlmPreset] =
        useState("describe_image");
    ...
    const existingVlmRuns = useMemo(() => {
        ...
        return Object.entries(
            rawRuns as Record<
                string,
                { effective_prompt?: string; tags?: string[] }
            >,
                ...
                <div className="custom-scrollbar flex-1 space-y-6 overflow-y-auto p-4">
                    <div className="space-y-4">
                        ...
                        <div>
                            ...
                            <div className="flex items-center space-x-2 rounded-sm border border-border bg-input p-2">
                                ...
                                <button
                                    onClick={() =>
                                        copyToClipboard(image.asset_id)
                                    }
                                    className="text-muted-foreground hover:text-foreground"
                                >
                                    <Copy className="h-3 w-3" />
                        ...
                        <div>
                            ...
                            >
                                <input
                                    ...
                                    onChange={(event) =>
                                        setCornerCasePrompt(event.target.value)
                                    }
                    ...
                    {isExoSourceImage(image) && (
                        <div>
                            ...
                            <form
                                onSubmit={generateEgoView}
                                className="space-y-4"
                            >
                                <select
                                    value={selectedCameraWork}
                                    onChange={(event) =>
                                        setSelectedCameraWork(
                                            event.target.value,
                                        )
                                    }
                                    className="w-full rounded-sm border border-border bg-input px-3 py-2 text-sm focus:border-primary/50 focus:outline-none"
