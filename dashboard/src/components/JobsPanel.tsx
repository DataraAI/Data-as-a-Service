...
export function JobsPanel({ open, onOpenChange, canViewHistory }: JobsPanelProps) {
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent
        side="right"
        className="flex w-[min(100vw,420px)] flex-col gap-0 overflow-hidden border-slate-200 bg-white p-0 text-slate-950 sm:max-w-[420px]"
      >
  ...
  return (
    <section className="mb-5">
      ...
      {jobs.length === 0 && emptyText ? (
        <div className="rounded-sm border border-slate-200 bg-white px-3 py-3 text-xs text-slate-500">
          {emptyText}
        </div>
      ) : (
...
function JobCard({ job, showOwner, muted }: { job: LambdaJob; showOwner: boolean; muted: boolean }) {
  ...
  return (
    <article className={`rounded-sm border p-3 ${muted ? "border-slate-200 bg-slate-50" : "border-primary/20 bg-primary/5"}`}>
      ...
      <div className="mt-3 grid grid-cols-2 gap-2 text-[11px]">
        <div className="rounded-sm bg-white/75 px-2 py-1.5">
