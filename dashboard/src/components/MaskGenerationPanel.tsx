    ...
    if (job.status === "succeeded") {
      ...
      if (typeof job.video_count === "number" && job.video_count > 0) {
        descriptionParts.push(
          `${job.video_count} src_cam overlay ${job.video_count === 1 ? "video was" : "videos were"} uploaded to the dataset-level videos folder.`,
        );
      }
            ...
            {expandedSections.handMotion && (
              <div className="space-y-3 bg-background/30 px-4 pb-4">
                <p className="text-[11px] leading-relaxed text-muted-foreground">
                  ...
                  meshes are stored under the dataset-level hand_meshes folder, and the rendered
                  src_cam overlay lands in the dataset-level videos folder in Azure.
                </p>
                <div className="rounded-sm border border-primary/20 bg-primary/5 px-3 py-2 text-[11px] text-muted-foreground">
                  Uses the full 30fps frame sequence from this leaf ego folder and uploads OBJ
                  meshes plus a src_cam overlay video.
                </div>
                {handMotionJob && (
                  <div className={`rounded-sm border px-3 py-2 ${handMotionStatusClasses}`}>
                    ...
                    {typeof handMotionJob.video_count === "number" && handMotionJob.video_count > 0 && (
                      <div className="mt-1 text-[11px] text-muted-foreground">
                        Videos: {handMotionJob.video_count}
                      </div>
                    )}
                    {handMotionJob.error && (
                        ...
                        {typeof handMotionJob.output_viewer_path === "string" &&
                          handMotionJob.output_viewer_path.trim() && (
                            <button
                              ...
                              className="font-sans-tech text-[11px] text-primary underline-offset-4 hover:underline"
                            >
                              Open meshes
                            </button>
                          )}
                        {typeof handMotionJob.output_video_viewer_path === "string" &&
                          handMotionJob.output_video_viewer_path.trim() && (
                            <button
                              type="button"
                              onClick={() => onOpenViewerPath(handMotionJob.output_video_viewer_path as string)}
                              className="font-sans-tech text-[11px] text-primary underline-offset-4 hover:underline"
                            >
                              Open video
                            </button>
